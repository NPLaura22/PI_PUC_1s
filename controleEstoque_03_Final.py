...
def calcular(CodProduto, CP, CF, CV, IV, ML, conexao):
    a=CF+CV+IV+ML
    if a >= 100:
        print("Erro: A soma de CF, CV, IV e ML deve ser menor que 100% para evitar divisão por zero.")
        return
    else:
        b=1-(a/100)
        PV=CP/b
        ...
        print("\n\nA) Preço de Venda: R${:.2f} \n".format(PV))

        print("B) Custo de Aquisição (Fornecedor): R${} -> {:.2f}%\n".format(CP, CP2))

        print("C) Receita Bruta (A-B): R${:.2f} -> {:.2f}%\n".format(RB, RB2))

        print("D) Custo Fixo/Administrativo: R${:.2f} -> {}%\n".format(CF2, CF))

        print("E) Comissão de Vendas: R${:.2f} -> {}%\n".format(CV2, CV))

        print("F) Impostos: R${:.2f} -> {}%\n".format(IV2, IV))

        print("G) Outros custos (D+E+F): R${:.2f} -> {:.2f}%\n\n".format(OC, OC2))
        
        try:
            print("H)Rentabilidade (C-G): R${:.2f} -> {:.2f}%".format(rent, rent2))
            if rent2>=20:
                print("Lucro Alto")
            ...
            if not resultados:
                ...
                resultado = (CodProduto, PV,CP, RB, CF2, CV2, IV2, OC, rent, tipoLucro)
